...
$("#add").click(function(){
	...
	$("#get_purchasereturn_data").delegate(".select_supplier","change",function()	
	{
	...
	$("#sales_return_form").click(function(){
		var invoice = $("get_purchasereturn_data").serialize();
			if($("#invoice_no").val() === ""){
		...
		({
			url : DOMAIN+"/includes/process.php",
			method : "POST",
			data : $("#get_purchasereturn_data").serialize(),
			success : function(data)
			{
						...
						if(trimmedResponse<0 )
					{
						...
						$("#get_purchasereturn_data").trigger("reset");
